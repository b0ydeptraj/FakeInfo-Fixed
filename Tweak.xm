...
// MARK: - Factory Reset (Full Data Nuke - from MaxIdentity)
- (void)confirmFactoryReset {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Factory Reset"
        message:@"DELETE ALL app data: Documents, Library, Caches, Keychain, Cookies, UserDefaults. App will restart as fresh install. Cannot be undone!"
        preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
...
- (void)executeFactoryReset {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        for (id secClass in secClasses) {
            ...
            if (orig_SecItemDelete_ptr) orig_SecItemDelete_ptr((__bridge CFDictionaryRef)q);
            else SecItemDelete((__bridge CFDictionaryRef)q);
        }
